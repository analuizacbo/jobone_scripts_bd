--------------------------------------------------------
--  DDL for Package Body IT_CIGAM_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "IT_CIGAM_PKG" IS
 --
 ...
 BEGIN
  ...
  IF v_servico IS NULL OR upper(v_servico) <> 'NF_SAIDA_PROCESSAR' THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Código do serviço inválido (' || v_servico || ').';
   RAISE v_exception;
  END IF;
  --
 ...
 ------------------------------------------------------------------------------------------
  ...
  -- ALTERADO POR      DATA        MOTIVO ALTERACAO
  -- ----------------  ----------  ---------------------------------------------------------
  -- Silvia            11/04/2017  Inclusao de flag_simples e flag_cpom
  ------------------------------------------------------------------------------------------
 (
  p_sistema_externo_id IN sistema_externo.sistema_externo_id%TYPE,
  p_empresa_id         IN empresa.empresa_id%TYPE,
 ...
 BEGIN
  ...
  SELECT xmlagg(xmlelement("corpo",
                           ...
                           xmlelement("flag_simples", flag_simples),
                           xmlelement("flag_cpom", flag_cpom)))
    INTO v_xml_corpo
    FROM pessoa
   WHERE pessoa_id = p_pessoa_id;
  --
 ...
 ------------------------------------------------------------------------------------------
  ...
  -- ALTERADO POR      DATA        MOTIVO ALTERACAO
  -- ----------------  ----------  ---------------------------------------------------------
  -- Silvia            18/07/2016  Mudancas na integracao
  ------------------------------------------------------------------------------------------
 (
  p_sistema_externo_id IN sistema_externo.sistema_externo_id%TYPE,
  p_empresa_id         IN empresa.empresa_id%TYPE,
  ...
  v_empresa_fat            pessoa.apelido%TYPE;
  v_cod_ext_cliente        pessoa_sist_ext.cod_ext_pessoa%TYPE;
  v_cod_ext_job            job.cod_ext_job%TYPE;
  v_cod_emp_fatur          empr_fatur_sist_ext.cod_ext_fatur%TYPE;
  v_cod_tipo_finan         tipo_financeiro.codigo%TYPE;
  v_num_estim              VARCHAR2(100);
  --
  CURSOR c_itn IS
   SELECT it.orcamento_id,
          decode(it.natureza_item, 'CUSTO', 'CUSTO', 'HONOR', 'HONOR', 'ENCARGO') AS natureza_item,
          nvl(SUM(ia.valor_fatura), 0) AS valor_fatura
     FROM item_fatur ia,
          item       it
    WHERE ia.faturamento_id = p_faturamento_id
      AND ia.item_id = it.item_id
    GROUP BY it.orcamento_id,
             decode(it.natureza_item, 'CUSTO', 'CUSTO', 'HONOR', 'HONOR', 'ENCARGO');
  --
 BEGIN
  v_qt := 0;
  --
  ...
  END IF;
  --
  IF v_cod_tipo_finan IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O código do tipo financeiro desse job não foi definido.';
   RAISE v_exception;
  END IF;
  --
  SELECT MAX(cod_ext_fatur)
    INTO v_cod_emp_fatur
    FROM empr_fatur_sist_ext
   WHERE sistema_externo_id = p_sistema_externo_id
     AND pessoa_id = v_emp_faturar_por_id;
  --
  IF v_cod_emp_fatur IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O código externo dessa empresa de faturamento não foi definido (' ||
                 v_empresa_fat || ').';
   RAISE v_exception;
  END IF;
  --
  ...
  -- monta a secao "corpo"
  ------------------------------------------------------------
  SELECT xmlconcat(xmlelement("codigo_empresa_fatur", v_cod_emp_fatur),
                   xmlelement("codigo_externo_faturamento", v_cod_ext_fatur),
                   xmlelement("codigo_externo_job", v_cod_ext_job),
                   xmlelement("tipo_financeiro", v_cod_tipo_finan),
  ...
  FOR r_itn IN c_itn
  LOOP
   v_num_estim := orcamento_pkg.numero_formatar(r_itn.orcamento_id);
   --
   SELECT xmlagg(xmlelement("item",
                            xmlelement("estimativa", v_num_estim),
 ...
 (
  ...
  v_xml_conteudo_itens     xmltype;
  v_xml_conteudo_itens_aux xmltype;
  v_xml_conteudo_dup       xmltype;
  v_xml_conteudo_dup_aux   xmltype;
  v_xml_out                CLOB;
  v_xml_in                 CLOB;
  v_cod_ext_empresa        empresa_sist_ext.cod_ext_empresa%TYPE;
  v_nome_empresa           empresa.nome%TYPE;
  ...
  CURSOR c_itn IS
   SELECT it.orcamento_id,
          nvl(io.tipo_produto_id, it.tipo_produto_id) AS tipo_produto_id,
          nvl(SUM(io.valor_aprovado), 0) AS valor_nota
     FROM item_nota io,
          item      it
    WHERE io.nota_fiscal_id = p_nota_fiscal_id
      AND io.item_id = it.item_id
    GROUP BY it.orcamento_id,
             nvl(io.tipo_produto_id, it.tipo_produto_id);
  --
  CURSOR c_dup IS
   SELECT duplicata_id,
          num_parcela,
 ...
 BEGIN
  ...
  END IF;
  --
  IF v_tipo_financeiro_id IS NOT NULL THEN
   SELECT TRIM(codigo)
  ...
  ------------------------------------------------------------
  SELECT xmlconcat(xmlelement("codigo_empresa_fatur", v_cod_emp_fatur),
                   xmlelement("codigo_externo_pagamento", v_cod_ext_nf),
                   xmlelement("codigo_externo_job", v_cod_ext_job),
                   xmlelement("tipo_financeiro", v_cod_tipo_finan),
                   ...
                   xmlelement("modo_pagamento", nvl(v_modo_pagto, 'OU')))
    INTO v_xml_corpo
    FROM dual;
  --
  -- monta a secao "itens"
  ...
  LOOP
   ...
   SELECT MAX(cod_ext_produto)
     ...
     FROM tipo_produto
    WHERE tipo_produto_id = r_itn.tipo_produto_id;
   --
   SELECT xmlagg(xmlelement("item",
                            xmlelement("estimativa", v_num_estim),
                            xmlelement("codigo_externo_produto", v_cod_ext_produto),
                            xmlelement("quantidade", '1'),
                            xmlelement("valor", numero_cigam_mostrar(r_itn.valor_nota, 2, 'N'))))
     INTO v_xml_conteudo_itens_aux
     FROM dual;
   --
   SELECT xmlconcat(v_xml_conteudo_itens, v_xml_conteudo_itens_aux)
  ...
  LOOP
   SELECT xmlagg(xmlelement("parcela",
                            xmlelement("data_venc", data_cigam_mostrar(r_dup.data_vencim)),
                            xmlelement("valor",
                                       numero_cigam_mostrar(r_dup.valor_duplicata, 2, 'N'))))
     INTO v_xml_conteudo_dup_aux
     FROM dual;
   --
   SELECT xmlconcat(v_xml_conteudo_dup, v_xml_conteudo_dup_aux)
     INTO v_xml_conteudo_dup
     FROM dual;
  END LOOP;
  --
  SELECT xmlagg(xmlelement("duplicatas", v_xml_conteudo_dup))
    INTO v_xml_conteudo_dup
    FROM dual;
  --
  -- junta o corpo com as duplicatas
  SELECT xmlconcat(v_xml_corpo, v_xml_conteudo_dup)
    INTO v_xml_corpo
    FROM dual;
  --
  SELECT xmlagg(xmlelement("corpo", v_xml_corpo))
 ...
 ------------------------------------------------------------------------------------------
  ...
  -- ALTERADO POR      DATA        MOTIVO ALTERACAO
  -- ----------------  ----------  ---------------------------------------------------------
  -- Silvia            09/05/2017  Integracao de estimativa (ORCAMENTO) usando o servico
  --                               de integracao de job.
  -- Silvia            23/06/2017  Novo parametro objeto_id
  ------------------------------------------------------------------------------------------
 (
  p_sistema_externo_id IN sistema_externo.sistema_externo_id%TYPE,
 ...
 BEGIN
  ...
  IF p_cod_objeto = 'PESSOA' THEN
   ...
   webservice_pkg.chamar(p_sistema_externo_id,
                         ...
                         p_erro_msg);
   --
  ELSIF p_cod_objeto IN ('JOB', 'ORCAMENTO') THEN
   --
   webservice_pkg.chamar(p_sistema_externo_id,
                         p_empresa_id,
                         'jobIntegrar',
